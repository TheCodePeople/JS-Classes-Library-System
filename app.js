...
class Library {
  ...
  checkOutBook(id, patron) {
    const request = this.getBookById(id);
    if (!request.status) {
      request.status = new BookStatus();
    }
    if (request && !request.status.checkedOut) {
      request.status.checkedOut = true;
      request.status.patron = patron;
    }
  }
  checkInBook(id) {
